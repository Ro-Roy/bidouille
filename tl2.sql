WITH filtered_reports AS (
    SELECT
        ...
        END AS duration -- Duration in seconds
    FROM napoleon_data.public_reports
    WHERE location IS NOT NULL -- Retain rows with missing timestamps for further tagging
),
ordered_reports AS (
    SELECT
        id,
...
timeline_analysis AS (
    SELECT
        ...
        prev_end_time,
        CASE 
            WHEN start_time IS NULL OR prev_end_time IS NULL THEN NULL
            ELSE EXTRACT(EPOCH FROM (start_time - prev_end_time))
        END AS time_difference, -- Difference in seconds
        CASE 
            WHEN latitude IS NULL OR prev_latitude IS NULL THEN NULL
            ELSE 6371 * acos(
        ...
        END AS distance -- Haversine formula for distance
...
tagged_issues AS (
    SELECT
        ...
        distance,
        CASE
            WHEN start_time IS NULL OR end_time IS NULL OR location IS NULL THEN 'missing_data'
